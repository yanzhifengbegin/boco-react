    ...
    axiosProxy.interceptors.response.use(
        ...
        (error) => {
            if (error.response && error.response.status === 401) {
                //忽略掉可以匿名访问的url
                let isMatchAnonyUrl = configs.urls.anonymousUrls.reduce((result, current) => {
                    if (!result) {
            ...
            get(target, propKey, receiver) {
                return (parameters) =>
                    new Promise((resolve, reject) => {
                        ...
                        let apiUrl = fullUrl || `${constants.APP_HOST || ''}/${apiPath}/${propKey}`;
                        axiosProxy({
                            method: type,
                            url: apiUrl,
                            data: axiosData,
                            ...
                            .then((resp) => {
                                if (showSuccessMessage) {
                                    const message = getSuccessMessage(type, defaultSuccessMessage);
                            ...
                            .catch((reason) => {
                                if (showErrorMessage) {
                                    const message = getErrorMessage(type, defaultErrorMessage || reason.message);
                                    message && AntdMessage.fail(message);
                                }
                                reject(reason);
                            });
                    });
